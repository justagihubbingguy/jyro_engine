...
import net.jyro.windows.gui.input.KeyboardUtils;
import net.jyro.windows.gui.input.MouseUtils;
import org.joml.Vector3f;
import org.lwjgl.glfw.GLFW;

import static org.lwjgl.system.windows.User32.Functions.PostMessage;
...
public class CameraWindows {
    ...
    public CameraWindows(WinDef.HWND handle) {
        this.window = handle;
    }
    ...
    public void move(float deltaTime) {
        ...
        if (KeyboardUtils.isKeyDown(KeyboardUtils.KEY_W))
            movement.add(forward.mul(velocity, new Vector3f()));

        if (KeyboardUtils.isKeyDown(KeyboardUtils.KEY_S))
            movement.sub(forward.mul(velocity, new Vector3f()));
        if (KeyboardUtils.isKeyDown(KeyboardUtils.KEY_D))
            movement.sub(right.mul(velocity, new Vector3f()));

        if (KeyboardUtils.isKeyDown(KeyboardUtils.KEY_A))
            movement.add(right.mul(velocity, new Vector3f()));

        if (KeyboardUtils.isKeyDown(KeyboardUtils.KEY_E))
            movement.y += velocity;

        if (KeyboardUtils.isKeyDown(KeyboardUtils.KEY_R))
            movement.y -= velocity;
        if (KeyboardUtils.isKeyDown(KeyboardUtils.KEY_ESCAPE))
    ...
    public void updateMouseLook() {
        ...
        MouseUtils.ApplyCurrentMousePositionInApplication(window,x,y);
